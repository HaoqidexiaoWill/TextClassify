...
from pytorch_transformers.modeling_bertSeqMatrix import BertForTokenClassification
...
from DATAProcess.LoadDATAMultiWOZ4 import DATAMultiWOZ
from Metric.ComputeMultiWOZ import accuracyF1,compute_jointGoal_domainslot_1_
...
class Trainer:
    ...
    def create_dataloader(self):
        ...
        all_label_sentence_domainslot = torch.tensor([f.label_sentence_domainslot for f in train_features], dtype=torch.float)
        all_label_tokens_domainslot = torch.tensor([f.label_tokens_domainslot for f in train_features], dtype=torch.float)
        ...
        eval_label_sentence_domainslot = torch.tensor([f.label_sentence_domainslot for f in eval_features], dtype=torch.float)
        eval_label_tokens_domainslot = torch.tensor([f.label_tokens_domainslot for f in eval_features], dtype=torch.float)
    ...
    def train(self):
        ...
        for step in range(num_train_optimization_steps):
            ...
            loss = loss_tokenstart + loss_tokenend + loss_sentence_domainslot
            tr_loss += loss.item()
            ...
            if self.do_eval and (step + 1) % (self.eval_steps * self.gradient_accumulation_steps) == 0:
                for file in ['dev.csv']:
                    ...
                    utterance_text = [x.text_history for x in eval_examples]
                    ...
                    for input_ids,input_mask, segment_ids,\
                        ...
                        # logits_tokens_start = torch.sigmoid(logits_tokens_start)
                        # logits_tokens_start = (logits_tokens_start > 0.4).float()
                        # logits_tokens_start = logits_tokens_start.cpu().long().numpy()
                        logits_tokens_start = logits_tokens_start.view(-1, 2).cpu().numpy()
                        logits_tokens_end = logits_tokens_end.view(-1,2).cpu().numpy()


                        logits_tokens_domainslot = logits_tokens_domainslot.view(-1,26).detach().cpu().numpy()

                        logits_sentence_domainslot = torch.sigmoid(logits_sentence_domainslot)
                        logits_sentence_domainslot = (logits_sentence_domainslot > 0.4).float()
                        logits_sentence_domainslot = logits_sentence_domainslot.cpu().long().numpy()


                        # logits_tokens_end = torch.sigmoid(logits_tokens_end)
                        # logits_tokens_end = (logits_tokens_end > 0.4).float()
                        # logits_tokens_end = logits_tokens_end.cpu().long().numpy()

                        # logits_tokens_end = logits_tokens_end.cpu().long().numpy()
                        # logits_tokens_end = np.argmax(logits_tokens_end,axis=-1)
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        ...
        output_dir = './model_MultiWOZ_1135',
        # DOUBAN 是二分类
