...
def read_examples_(input_file, is_training):
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, split_num,
    ...
    for example_index, example in enumerate(examples):
        ...
        '''
        这里采取的办法比较单一，无论句子长短，采用了统一的截断方法，这样不利于不同长度句子的判断
        可以采取一些手段，按照同一长度进行截断，不同长度句子的截断次数不同
        ...
        for i in range(split_num):
            ...
            label = example.label
            if example_index < 10 and is_training:
                logger.info("*** Example ***")
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default='./uncased_L-12_H-768_A-12/', type=str, required=False,
