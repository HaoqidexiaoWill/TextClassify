...
from oldcode.LoadDataBDCI import InputExample,InputFeatures
...
class Trainer:
    ...
    def create_dataloader(self):
        ...
        train_features = self.convert_examples_to_features(train_examples, self.tokenizer, self.max_seq_length)
        ...
        eval_examples = self.read_examples(os.path.join(self.data_dir, 'dev.csv'), is_training=True)
        eval_features = self.convert_examples_to_features(eval_examples, self.tokenizer, self.max_seq_length)
    ...
    def train(self):
        ...
        if self.do_test:
            ...
            gc.collect()
            self.do_train = False
            model = BertForSequenceClassification.from_pretrained(os.path.join(self.output_dir, "pytorch_model.bin"), self.args,
                                                                  config=config)
            model.to(self.device)
            ...
            for file, flag in [('dev.csv', 'dev'), ('test.csv', 'test')]:
                inference_labels = []
                gold_labels = []
                eval_examples = self.read_examples(os.path.join(self.data_dir, file), is_training=False)
                ...
                if flag == 'test':
                    df = pd.read_csv(os.path.join(self.data_dir, file),names = ['id', 'content', 'title', 'label'])
                    predict = np.argmax(logits, axis=1).tolist()
                    print(df.shape[0])
                    print(len(predict))
                    df['labelpre'] = predict
                    df[['id','labelpre']].to_csv(os.path.join(self.output_dir, "sub.csv"),index=False,header = False)
