            ...
            if self.do_eval and (step + 1) % (self.eval_steps * self.gradient_accumulation_steps) == 0:
                for file in ['dev.csv']:
                    ...
                    for input_ids, input_mask, segment_ids,label_domain,label_dependcy in eval_dataloader:
                        ...
                        logits_domain = logits_domain.view(-1, self.num_labels).detach().cpu().numpy()
                        logits_dependcy = logits_dependcy.view(-1, self.num_labels).detach().cpu().numpy()
                    ...
                    eval_accuracy_domain = accuracyF1(scores_domain, gold_labels_domain)
                    eval_accuracy_dependcy = accuracyF1(scores_dependcy, gold_labels_dependcy)
                    print(
