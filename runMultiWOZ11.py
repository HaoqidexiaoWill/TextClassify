...
from Metric.ComputeMultiWOZ3 import compute_jointGoal_domainslot
...
class Trainer:
    ...
    def train(self):
        ...
        for step in range(num_train_optimization_steps):
            ...
            label_sentence_domainslot = batch

            loss_tokenstart,loss_tokenend,loss_sentence_domainslot = model(
                input_ids=input_ids,
            ...
            if self.do_eval and (step + 1) % (self.eval_steps * self.gradient_accumulation_steps) == 0:
                for file in ['dev.csv']:
                    ...
                    for input_ids,input_mask, segment_ids,\
                        utterance_mask,domainslot_mask,\
                        label_tokens_start,label_tokens_end,\
                        label_sentence_domainslot in eval_dataloader:
                        ...
                        with torch.no_grad():
                            batch_eval_loss_tokens_start,batch_eval_loss_tokens_end,batch_eval_loss_sentence_domainslot = model(
                                input_ids=input_ids,
                                token_type_ids=segment_ids,
                                attention_mask=input_mask,
                                utterance_mask = utterance_mask,
                        ...
                        logits_sentence_domainslot = logits_sentence_domainslot.view(-1,2).detach().cpu().numpy()
    ...
    def test_eval(self):
        ...
        for input_ids, input_mask, segment_ids, \
            ...
            logits_tokens_start = logits_tokens_start.detach().view(-1, 2).cpu().numpy()
            logits_tokens_end = logits_tokens_end.detach().view(-1, 2).cpu().numpy()
            logits_sentence_domainslot = logits_sentence_domainslot.view(-1, 2).detach().cpu().numpy()
        ...
        assert   scores_tokens_start.shape[0] == scores_tokens_end.shape[0]  == gold_labels_tokens_start.shape[0] == gold_labels_tokens_end.shape[0]
        ...
        print(
            'F1_token_domainslot', F1_token_domainslot,
            'F1_sentence_domainslot', F1_sentence_domainslot,
            'eval_F1_tokenstart', test_F1_tokenstart,
