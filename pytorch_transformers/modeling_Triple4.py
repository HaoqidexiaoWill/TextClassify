...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(len(filter_sizes) * n_filters*3+config.hidden_size*3, config.num_labels)
    ...
    def forward(self,
        ...
        hist_rnn_cat = torch.cat((context_hist, hist_rnn), 2)
        utt_rnn_cat = torch.cat((context_utt, utt_rnn), 2)
        resp_rnn_cat = torch.cat((context_resp, resp_rnn), 2)
        ...
        hist_rnn_maxpooling = nn.functional.max_pool1d(hist_rnn_cat, hist_rnn_cat.size()[2]).squeeze(2)
