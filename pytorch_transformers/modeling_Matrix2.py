...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        utt_length = utterance_mask_output.size(1)
        domainslot_lenght = domainslot_mask_output.size(1)
        utterance_mask_output_expand = torch.unsqueeze(utterance_mask_output, 2).expand(
            utterance_mask_output.size(0),
            utterance_mask_output.size(1),
            domainslot_lenght,
            utterance_mask_output.size(2)
        )
        domainslot_mask_output_expand = torch.unsqueeze(domainslot_mask_output, 1).expand_as(
            utterance_mask_output_expand
        )
        # print(utterance_mask_output_expand.size())
        # print(domainslot_mask_output_expand.size())
        attention = torch.cat([utterance_mask_output_expand,domainslot_mask_output_expand],dim=-1)
        logit_attention = self.classifier_tokens_domainslot(attention)
        # print(logit_attention.size())
        # attention = torch.bmm(
        #     torch.unsqueeze(utterance_mask_output, 2),
        #     torch.unsqueeze(domainslot_mask_output, 1))
        ...
        if label_tokens_start is not None:
            ...
            loss_sentence_domainslot = loss_fct(
                logits_sentence_domainslot.view(-1, self.sentence_domainslot_numlabels),
                label_sentence_domainslot.view(-1))
            loss_token_domainslot = loss_fct(
                logit_attention.view(-1,self.token_domainslot_labels),
                label_tokens_domainslot.view(-1))
            ...
            return logits_tokenstart,logits_tokenend,logits_sentence_domainslot,logit_attention