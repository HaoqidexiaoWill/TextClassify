...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        context_utt_ = self.dot_product_attention(utterance_mask_output, history_mask_output, history_mask_output)
        context_utt_ = self.FeedForward2(context_utt_)
        context_utt = self.layer_norm(utterance_mask_output+context_utt_)

        # utt 对 resp 的attention 加权表示
        context_resp_ = self.dot_product_attention(response_mask_output, history_mask_output, history_mask_output)
        context_resp_ = self.FeedForward2(context_resp_)
        context_resp = self.layer_norm(response_mask_output+context_resp_)
        # print(context_resp.size())

        # resp 对 utt 的attention加权表示
        context_utt_ = self.dot_product_attention(context_utt, context_resp, context_resp)
        context_utt_ = self.FeedForward2(context_utt_)
        context_utt = self.layer_norm(context_utt+context_utt_)

        # utt 对 resp 的attention 加权表示
        context_resp_ = self.dot_product_attention(context_resp, context_utt, context_utt)
        context_resp_ = self.FeedForward2(context_resp_)
        context_resp = self.layer_norm(context_resp+context_resp_)
