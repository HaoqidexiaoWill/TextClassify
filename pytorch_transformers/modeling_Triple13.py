...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.transformer = TransformerEncoder(hidden_size=self.hidden_size)
    ...
    def forward(self,
        ...
        transformer_hist = self.transformer(context_hist)
        transformer_utt = self.transformer(context_utt)
        transformer_resp = self.transformer(context_resp)

        context_hist,context_resp,context_utt = self.triple_attention(
            history = transformer_hist,
            utterance = transformer_utt,
            response = transformer_resp
        )
        '''
        resnet_hist = self.resnet(context_hist.permute(0, 2, 1))
        resnet_utt = self.resnet(context_utt.permute(0,2,1))
