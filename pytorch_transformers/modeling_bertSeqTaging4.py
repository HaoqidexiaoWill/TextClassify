...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier_domainslot = nn.Sequential(
            nn.Linear(config.hidden_size,config.hidden_size),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(config.hidden_size,self.domainslot_labels)
        )
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                label_domainslot=None,label_domain=None,label_dependcy = None,
        ...
        logits_domainslot = self.classifier_domainslot(pooling)
        ...
        if label_domain is not None and label_dependcy is not None:
            ...
            loss_domainslot = loss_domainslot_fct(logits_domainslot.view(-1, 1), label_domainslot.view(-1, 1))
            ...
            return loss_domainslot,loss_domain,loss_dependcy
        else:
            # logits_domainslot = nn.functional.softmax(logits_domainslot, -1)
            logits_domain = nn.functional.softmax(logits_domian, -1)
            logits_dependcy = nn.functional.softmax(logits_dependcy, -1)
            return logits_domainslot,logits_domain,logits_dependcy