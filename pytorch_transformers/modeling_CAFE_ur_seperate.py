...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, utt_input_ids, resp_input_ids, utt_token_type_ids=None, resp_token_type_ids=None, utt_attention_mask=None, resp_attention_mask=None, labels=None,
                position_ids=None, head_mask=None):

        utt_flat_input_ids = utt_input_ids.view(-1, utt_input_ids.size(-1))
        # utt_flat_position_ids = position_ids.view(-1, position_ids.size(-1)) if position_ids is not None else None
        utt_flat_position_ids = None
        utt_flat_token_type_ids = utt_token_type_ids.view(-1, utt_token_type_ids.size(-1)) if utt_token_type_ids is not None else None
        utt_flat_attention_mask = utt_attention_mask.view(-1, utt_attention_mask.size(-1)) if utt_attention_mask is not None else None

        resp_flat_input_ids = resp_input_ids.view(-1, resp_input_ids.size(-1))
        # resp_flat_position_ids = position_ids.view(-1, position_ids.size(-1)) if position_ids is not None else None
        resp_flat_position_ids = None
        resp_flat_token_type_ids = resp_token_type_ids.view(-1, resp_token_type_ids.size(-1)) if resp_token_type_ids is not None else None
        resp_flat_attention_mask = resp_attention_mask.view(-1, resp_attention_mask.size(-1)) if resp_attention_mask is not None else None

        utt_encoded_layers, _ = self.bert(input_ids=utt_flat_input_ids, position_ids=utt_flat_position_ids,
                                      token_type_ids=utt_flat_token_type_ids,
                                      attention_mask=utt_flat_attention_mask, head_mask=head_mask)
        resp_encoded_layers, _ = self.bert(input_ids=resp_flat_input_ids, position_ids=resp_flat_position_ids,
                                      token_type_ids=resp_flat_token_type_ids,
                                      attention_mask=resp_flat_attention_mask, head_mask=head_mask)
        utt_encoded_layers = self.dropout(utt_encoded_layers) # batchsize*seqlen=128*embdim=768
        resp_encoded_layers = self.dropout(resp_encoded_layers) # batchsize*seqlen=128*embdim=768
        ...
        utt_maxpooling, resp_maxpool = self.cafe(utt_encoded_layers,resp_encoded_layers)
