...
class InputFeatures(object):
    def __init__(self,example_id,hist_token,choices_features,
                 ...
                 label_sentence_domainslot,label_tokens_domainslot,
                 ):
...
class DATAMultiWOZ:
        ...
        with open(input_file, encoding='utf-8') as inf:
            for idx, line_ in enumerate(inf):
                ...
                examples.append(InputExample(
                    ...
                    label_tokens_domainslot = row['label_tokens_domainslot']
                ))
            return examples
    ...
    def convert_examples_to_features(self,examples, tokenizer, max_seq_length):
        ...
        for example_index, example in enumerate(examples):
            ...
            assert len(eachturn_label_tokens_domainslot) == len(eachturn_label_tokens_start) == len(eachturn_label_tokens_end)
            assert len(eachturn_label_tokens_domainslot[0]) == len(eachturn_label_sentence_domainslot)
